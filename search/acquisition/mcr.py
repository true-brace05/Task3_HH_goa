...
class MCRAcquisitionProvider(ImageAcquisitionProvider):
    def __init__(self):
        self.name = "mcr-acquisition"
        self.registry = "mcr.microsoft.com"
        self.registry_url = "https://mcr.microsoft.com/v2"

    def can_acquire(self, candidate: dict) -> bool:
        if not candidate.get("image_url"):
            return False
        return candidate.get("provider") == "microsoft-foundry" or self.registry in candidate.get("image_url", "")
    ...
    def acquire(self, candidate: dict) -> dict:
        ...
        # Parse repository name from the MCR URL
        # e.g., https://mcr.microsoft.com/samples/blockchain-ai/0xdeca10b-demo
        repo_path = image_url.replace(f"https://{self.registry}/", "")
        if not repo_path:
            return {
                "candidate_id": candidate_id,
                "status": "failed",
                "error": "Could not parse repository path from image_url",
                "provider": self.name,
            }

        logger.info("MCR acquisition for repo: %s", repo_path)

        # Try to get the manifest to find blob digests
        try:
            manifest_url = f"{self.registry_url}/{repo_path}/manifests/latest"
            ...
            return {
                "candidate_id": candidate_id,
                "status": "failed",
                "error": f"Failed to get manifest: {e}",
                "provider": self.name,
            }

        # Get the first layer blob digest
        ...
        blob_url = f"{self.registry_url}/{repo_path}/blobs/{blob_digest}"

        logger.info("Attempting to download blob: %s", blob_url)
        ...
        for attempt in range(RETRIES):
            try:
                ...
                if content_type != "application/octet-stream" and "image" not in content_type:
                    pass  # Not necessarily a failure - could be valid
                ...
                # Try to download and validate as image
                content = br.content
                try:
                    ...
                    # Not a visual image - it's a container layer
                    return {
                        "candidate_id": candidate_id,
                        "status": "failed",
                        "error": "Blob is not a visual image — container image layer (not a photograph)",
                        "provider": self.name,
